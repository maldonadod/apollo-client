...
>(
  ...
  useEffect(() => {
    if (client.disableNetworkFetches || context.renderPromises) {
      return;
    }
  ...
  useEffect(() => {
    if (context.renderPromises && result.loading && ssr !== false) {
      const queryData: QueryData1<TData, TVariables> = {
        getOptions() {
          return options;
        },
  ...
  }, [result, context.renderPromises, ssr, options,]);
